...
export default function HistoriquePage() {
  ...
  const filtered = records.filter((r) => {
    ...
    const matchFilter =
      filter === "all" ||
      r.documentType === filter ||
      (filter === "facture" && r.documentType === "facture_libre");
        ...
        {filtered.length === 0 ? (
          ...
          <div className="space-y-3">
            {filtered.map((record, idx) => {
              ...
              const isFactureLibre = record.documentType === "facture_libre";
              const hasRemise = record.totalNet !== record.totalBrut && !isFactureLibre;
                ...
                >
                  ...
                  <div className="hidden sm:block px-5 py-3.5">
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      >
                        {isFactureLibre ? "Facture libre" : isFacture ? "Facture" : "Devis"}
                      </span>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        ...
                        <button
                          ...
                          className="p-2 rounded-lg text-ink-muted hover:text-[#0071E3] hover:bg-[rgba(0,113,227,0.08)] transition-colors"
                          title="Modifier"
                        >
                          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        ...
                        <button
                          onClick={() => isFactureLibre ? router.push("/facture-libre?duplicate=" + record.id) : handleDuplicate(record)}
                  ...
                  <div className="sm:hidden p-4">
                    ...
                    <div className="flex items-center gap-2 mt-3 pt-3 border-t border-surface-border">
                      ...
                      <button
                        onClick={() => router.push(isFactureLibre ? "/facture-libre?edit=" + record.id : "/dashboard?edit=" + record.id)}
                        className="flex-1 flex items-center justify-center h-10 rounded-xl bg-surface-muted text-ink-muted active:text-[#0071E3] active:bg-[rgba(0,113,227,0.08)] transition-colors"
                        title="Modifier"
                      >
                        <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                          <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>
                      ...
                      <button
                        onClick={() => isFactureLibre ? router.push("/facture-libre?duplicate=" + record.id) : handleDuplicate(record)}
                        className="flex-1 flex items-center justify-center h-10 rounded-xl bg-surface-muted text-ink-muted active:text-brand-orange active:bg-brand-orange-light transition-colors"
                        title="Dupliquer"
                      >
                        <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                          <rect x="9" y="9" width="13" height="13" rx="2" ry="2"/>
